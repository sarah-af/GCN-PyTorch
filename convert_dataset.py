        ...
        for line_num, line in enumerate(tqdm(f, desc="Converting molecules"), 1):
            try:
                ...
                if use_embeddings:
                    ...
                    with torch.no_grad():
                        node_features = embedding_layer(node_feats).numpy().tolist()
                else:
                ...
                graph_data = {
                    ...
                    'y': mol_data['y'][0] if isinstance(mol_data['y'], list) else mol_data['y']
                }
                
                # Add edge attributes if they exist
                if 'edge_attr' in mol_data:
                    graph_data['edge_attr'] = mol_data['edge_attr']
